...
description: Pre-install / pre-publish manifest linter for construct packs. Emits Verdict stream rows on findings. Checks required manifest fields, path resolution, route declarations, and the AGENTS.md grimoires-section convention.
---
...
5. **Grimoires-section convention** — `AGENTS.md` must contain an explicit `grimoires/<path>` read/write declaration so the pack's interface contract is legible
...
`AGENTS.md` must reference `grimoires/<path>` AND include at least one of: `Writes to`, `Reads from`, `writes:`, `reads:`. This is the convention the `construct-base` template enforces; installed packs that pre-date that template tend to drift.
...
# construct-validate · /path/to/packs/example-pack
  ...
  [medium] [grimoires_section] AGENTS.md contains no grimoires/ path reference
    → /path/to/packs/example-pack/AGENTS.md
