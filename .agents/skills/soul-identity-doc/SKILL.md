...
description: L7 soul-identity-doc — schema + SessionStart hook for descriptive `SOUL.md` (descriptive identity complement to prescriptive `AGENTS.md`). Composes with audit-envelope (soul.surface event), context-isolation-lib (sanitize body at SessionStart surfacing), and the prescriptive-rejection pattern file (NFR-Sec3 enforcement).
context: scoped
...
descriptive identity document distinct from prescriptive `AGENTS.md`. The
...
| Surfaced via | Native AGENTS.md loading | SessionStart hook + sanitize_for_session_start |

The two layers are intentionally separate. `AGENTS.md` is the *rules layer* —
...
- Documenting *what we are not* alongside what we are — a sharper boundary
  than AGENTS.md's prescriptive rules can carry.

## When NOT to use

- Imperative rules (`MUST do X`) → `AGENTS.md` instead.
...
| 7C | SKILL.md + CLI + cross-primitive integration tests + lore + AGENTS.md | SHIPPED |
