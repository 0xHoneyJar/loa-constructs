...
#   - Content inside counterfactual sections (Near Miss / Category Error H3s)
#   - Content under "# Example" headings (tables, prose — until next non-Example heading)
is_in_example_block() {
  local file="$1" target_line="$2"
  local in_fence=false
  local in_example_section=false
  local in_counterfactual_section=false
  local line_num=0
  ...
  while IFS= read -r line; do
    ...
    # Track headings (outside fences only — fenced headings are content)
    if [[ "$in_fence" == false ]] && [[ "$line" =~ ^#{1,6}[[:space:]] ]]; then
      # Example sections: any heading containing "Example"
      ...
      # Counterfactual sections: specific H3s (Near Miss / Category Error)
      if [[ "$line" =~ ^###.*Near[[:space:]]Miss ]] || [[ "$line" =~ ^###.*Category[[:space:]]Error ]]; then
        in_counterfactual_section=true
      elif [[ "$line" =~ ^### ]]; then
        # Any other H3 ends counterfactual (e.g., "### The Target")
        in_counterfactual_section=false
      elif [[ "$line" =~ ^## ]] || [[ "$line" =~ ^#[[:space:]] ]]; then
        # H1 or H2 also ends counterfactual
        in_counterfactual_section=false
      fi
    fi
    ...
    if [[ $line_num -eq $target_line ]]; then
      # Exempt if inside example or counterfactual section (fenced or prose)
      if [[ "$in_example_section" == true ]] || [[ "$in_counterfactual_section" == true ]]; then
        return 0
      fi
      return 1
    fi
