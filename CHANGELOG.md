...
- **construct-clew Sprint 2 (cycle-052)**: the cold-path distill reducer (`scripts/clew/distill.sh`). Reads un-distilled ledger lines → clusters by skill → runs the **generality** (FR-3) + **redaction** (FR-8) gates → fuzzy-matches `line_hint` against the target `SKILL.md` → emits an **inert** `PROPOSAL.diff` + a redacted `RATIONALE.md` to `skills-pending/`, stamping `distilled_at` idempotently under the shared lock. The verbatim operator quote is **structurally excluded** from every export; secret detection is fail-closed (broadened patterns + entropy fallback) across all exported fields. FR-8/FR-3 gates were hardened across 4 cross-model dissents + a 5-skeptic red-team (10 blocking findings, all fixed + regression-pinned; 20 bats tests). The mechanical gate is defense-in-depth — the human ratify gate (Sprint 3) is the final content control. (`feat/construct-clew`)
- **construct-clew Sprint 3 (cycle-052)**: the human-gated cross-repo half — ratify → propagate → surface (`scripts/clew/ratify.sh`, `propose-construct-learning.sh`, `surface.sh`; 29 bats tests). `ratify.sh approve` is the force-chain gate (a `PROPOSAL.diff` is inert until run); it drafts a **draft PR** to the *targeted construct's own* canonical repo resolved from `construct.yaml::repository.url` — never base-Loa (mirrors `proposal-generator.sh`; the vendored copy is untouched). The propagate gate is non-bypassable and ordered FR-6→FR-5: **C6 `isArchived` guard** runs first (archived → SKIP, write `distill_status=skipped_archived`, emit an uninstall flag *consumed by* `surface.sh flags`, PROMPT the operator, exit 0) then the **C5 `gh auth` pre-flight** (not-authed / 403 → exit 4 *before* any `gh pr create`). PR bodies are assembled from the already-redacted `RATIONALE.md` + inert diff only (FR-8 carry — no verbatim trigger). C7 surfacing (L6 handoff + SessionStart line + uninstall-flag reader) degrades to silence and re-surfaces a set ≤once/session (anti-bureaucracy §4.3). Mandatory tests #3/#4/#8/#12 green; `gh` injected via `LOA_CLEW_GH` stub. Genuine capture→distill→propose `--dry-run` chain proven on the **live** `artisan` construct. **Live finding:** the SDD's named `observer` pilot is actually archived (`construct-observer`→`construct-beehive`, `isArchived:true`) — the guard correctly SKIPs it; `artisan` is the live pilot. `/skill-audit --approve` command + SessionStart hook registration remain deferred System-Zone steps. (`feat/construct-clew`)
