...
|--------|------|---------|-------|
| `composition` | `runtime/composition.schema.json` | **1.4** | v1.1 added 2026-04-27: `surface_class`, `thinking_effort`, `vocabulary_governance`, `codex_mode`. Refactor: HivemindLabels extracted then migrated to loa-hivemind. v1.2 catches schema up to registry reality — 11 stage-level field shapes, output `emit_when`, top-level `changelog`/`depends_on`/`upstream_composition`, mode `blocking`, 4 new role enums, half-stage numbers, relaxed patterns. v1.3 adds `Stage.hitl_by_nature`. v1.4 adds `Stage.intelligence_tier` (cheap|standard|deep). All additive. |
...
| 1.3 | 2026-06-01 | Add optional `Stage.hitl_by_nature` boolean (cycle-053, authorized C053.OP-S1) — the compose-as-CC-workflow cut algorithm reads it to mark a stage the operator inherently performs by hand outside the agent loop (a third seam class alongside `mode:blocking` and the gate roles). | ✅ All registry YAMLs validate; v1.0/v1.1/v1.2 YAMLs validate unchanged. | (this PR) |
| 1.4 | 2026-06-03 | Add optional `Stage.intelligence_tier` (`cheap`|`standard`|`deep`) per RFC 2026-06-03-intelligence-router-coherence — read by the compose-as-CC-workflow runtime (construct-rooms-substrate segment-emitter) to route a segment's model by declared intelligence (cheap→haiku, standard→sonnet, deep→opus); absent → default-by-role. | ✅ All registry YAMLs validate; v1.0–v1.3 YAMLs validate unchanged. | (this PR) |
