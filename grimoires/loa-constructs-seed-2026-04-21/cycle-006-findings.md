...
32. **Question primitive as 6th stream type** (cycle-006 L-meta-pack emergence) — agent→operator structured solicitation (parallel to operator→agent Intent). Would formalize the pattern that Claude Code's AskUserQuestion already implements runtime-side. Skills emit structured questions; frontends render via native affordances (AskUserQuestion / form widget / TTS / readline). Breaks the frontend-swap invariant to hardcode any one rendering. Doctrine extension candidate for cycle-007; MVP (construct-creator creating-constructs skill) handles via prose-documented questions + runtime-chosen rendering.
