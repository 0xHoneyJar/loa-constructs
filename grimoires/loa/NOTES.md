...
- **Upstream bugs FILED (`0xHoneyJar/loa`)**: [#978](https://github.com/0xHoneyJar/loa/issues/978) `flatline-orchestrator.sh` mktemp BSD/macOS non-expansion; [#979](https://github.com/0xHoneyJar/loa/issues/979) cycle-110 §3.2 `auth_type`/`dispatch_group` not migrated to custom `.loa.config.yaml` providers. Memory `project_cheval_subscription_routing.md` updated (recurrence root-cause = uncommitted fix evaporated → now committed `6cf197eb`).
- **Layer-2 still open (GPT voice)**: `flatline_protocol.models.secondary: gpt-5.2` routes bare→http_api→`insufficient_quota`. Fix (not applied — routing change): `secondary: codex-headless:gpt-5.5`. Opus + Gemini voices work on subscription; only GPT http_api is quota-blocked. Applying it → full 3-voice Flatline.

### SDD cross-review (gemini-3.1-pro, 2026-06-01) — 2 real defects fixed, 1 Gemini overstatement caught
SDD written (`sdd-cycle-053-compose-onramp-hardening.md`, 409→~450 lines) then cross-model reviewed via direct cheval. **Verified facts** (verify-don't-trust): `schema-validator.sh` = bash/jq **partial** validator, `0`=valid/`1`=invalid, called by `compose-run.sh:143` + downstream `validate.yml:57` (`if !`); `validate-strict.ts` = **Ajv2020** full validator (the real "full validator" FR-5 wanted); `$schema`=draft/2020-12.
- **F2 CONFIRMED + sharpened**: PRD/brief's "exit 3 = valid" is **ungrounded** (absent from compose scripts) AND breaks the downstream `if !` (exit 3 = failure). → corrected PRD+SDD to **0/1**.
- **F3 CONFIRMED**: SDD §4.2 conflated "pin `schema-validator.sh`" (bash partial) with "python-jsonschema full validator." → FR-5 revised to adopt the existing `Ajv2020 validate-strict.ts`; python-jsonschema = fallback only.
- **F1 REAL**: "un-deadlockable" proof only covered FR-1 schema, not FR-5 validator strictness (shared `schema-validator.sh` is live downstream@main). → validator-strictness caveat added to §5.
- **F5 HALF**: `.known-broken/` works for the bash job (`**` skips dot-dirs) but NOT ajv `walkYaml` (Node readdir descends dot-dirs). → dot-skip required in `validate-strict.ts:95`.
- **F4 REFUTED**: no Draft-07 in path ($schema 2020-12, ajv=Ajv2020). Gemini confabulated (no repo access).
- OQ-A reframed: exit contract fixed at 0/1; remaining choice = adopt-ajv (rec) vs python for the local gate. **Next: /sprint-plan** (sprint-69/70/71 under cycle-053).
