...
| 3 | "No regressions in CI bats matrix on both Darwin and Linux runners" | ⚠ Partial | All tests pass on Darwin (this host). Linux verification pending CI run after operator pushes `.github/workflows/bats-tests.yml` (requires workflow OAuth scope agent lacks) — see Known Limitations |
