...
```
[observer](/constructs/observer) WRITES → grimoires/laboratory/canvases/
[artisan](/constructs/artisan)  READS  ← grimoires/laboratory/canvases/
[crucible](/constructs/crucible) READS  ← grimoires/laboratory/journeys/
[hardening](/constructs/hardening) READS ← grimoires/laboratory/
```
...
```
[observer](/constructs/observer) EMITS  → forge.observer.canvas_created
[crucible](/constructs/crucible) CONSUMES ← forge.observer.journey_shaped
[artisan](/constructs/artisan)  CONSUMES ← forge.observer.canvas_created
```
...
```
[protocol](/constructs/protocol) DEPENDS_ON → [observer](/constructs/observer), [artisan](/constructs/artisan)
...
| Governance as component | WORKING | `governs` / `governed_by` fields. Two roots: [vocabulary-bank](/constructs/vocabulary-bank), [artisan](/constructs/artisan). |
| Identity as component | WORKING | persona.yaml + narrative .md files. 5 deep, 22 shallow. |
| Verification as component | BUILT | Echelon pipeline: `construct_verifications` table + 3 API endpoints. See [Echelon](/verification/echelon). |
...
← [Index](/network/) · [Topology](/architecture/topology) · [Operator OS](/network/operator) · [Personas](/network/personas) · [Echelon](/verification/echelon)
