    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <div>
        <div className="flex items-center gap-3 mb-2 flex-wrap">
          <h1 className="text-3xl font-mono font-bold text-bone-base">{construct.name}</h1>
          <span className="border border-void-border px-2 py-0.5 text-sm font-mono text-bone-dim">
            v{construct.version}
          </span>
          <span className="text-sm font-mono text-bone-ghost uppercase">{construct.type}</span>
          {construct.constructType && construct.constructType !== 'skill-pack' && (
            <span className="border border-cyan-dim/30 bg-cyan-dim/10 px-2 py-0.5 text-sm font-mono text-cyan-dim">
          ...
          {(() => {
            ...
            if (tier === 'PROVEN') {
              return (
                <span className="border border-graduation-stable/30 bg-graduation-stable/10 px-2 py-0.5 text-xs font-mono text-graduation-stable">
                  Proven
                </span>
              );
            }
            if (tier === 'BACKTESTED') {
              return (
                <span className="border border-graduation-beta/30 bg-graduation-beta/10 px-2 py-0.5 text-sm font-mono text-graduation-beta">
                  Backtested
                </span>
              );
            }
            return (
              <span className="border border-void-border px-2 py-0.5 text-sm font-mono text-bone-ghost">
                Unverified
              </span>
            );
          })()}
        </div>
        <p className="text-base font-mono text-bone-dim">{construct.description}</p>
        {construct.longDescription && (
          <p className="text-base font-mono text-bone-ghost mt-2">{construct.longDescription}</p>
        )}
        {construct.forkedFrom && (
          <div className="mt-2">
            <Link
              href={`/constructs/${construct.forkedFrom.slug}`}
              className="inline-flex items-center gap-1 text-sm font-mono text-cyan-dim hover:text-cyan-base transition-colors"
            >
        ...
        {construct.forkCount > 0 && (
          <p className="text-sm font-mono text-bone-ghost mt-1">
      ...
      {construct.skillProse && (
        <div className="border border-void-border p-4">
          <p className="text-base font-mono text-bone-dim whitespace-pre-wrap">
            {construct.skillProse}
          </p>
        </div>
      )}
      ...
      {construct.verificationTier && construct.verificationTier !== 'UNVERIFIED' && (
        <div>
          <h2 className="text-base font-mono font-bold text-bone-base mb-3">Verification</h2>
          <div className="border border-void-border p-4">
            <div className="flex items-center gap-3 mb-2">
              <span className="text-sm font-mono text-bone-ghost">Tier</span>
              <span className="text-sm font-mono text-bone-base capitalize">
                {construct.verificationTier.toLowerCase()}
              </span>
            </div>
            {construct.verifiedAt && (
              <p className="text-sm font-mono text-bone-ghost">
      ...
      {/* Install */}
      <div className="border border-void-border p-4">
        <p className="text-sm font-mono text-bone-ghost mb-2">Install</p>
        <code className="block text-base font-mono text-cyan-base">
      ...
      {construct.commands.length > 0 && (
        <div>
          <h2 className="text-base font-mono font-bold text-bone-base mb-3">Commands</h2>
      ...
      {(construct.skills?.length ?? 0) > 0 && (
        <div>
          <h2 className="text-base font-mono font-bold text-bone-base mb-3">Skills</h2>
          <div className="space-y-2">
      ...
      {construct.composesWith.length > 0 && (
        <div>
          <h2 className="text-base font-mono font-bold text-bone-base mb-3">Composes With</h2>
          <div className="flex flex-wrap gap-2">
            {construct.composesWith.map((dep) => (
              <Link
                key={dep}
                href={`/constructs/${dep}`}
                className="border border-void-border px-2 py-1 text-sm font-mono text-bone-dim hover:bg-void-surface hover:text-bone-base transition-colors"
              >
                {dep}
              </Link>
            ))}
      ...
      {construct.showcases.length > 0 && (
        <div>
          <h2 className="text-base font-mono font-bold text-bone-base mb-3">Built With</h2>
          <div className="space-y-2">
      ...
      {construct.accuracy && (
        <div>
          <h2 className="text-base font-mono font-bold text-bone-base mb-3">Signal Accuracy</h2>
      ...
      {/* Links */}
      <div>
        <h2 className="text-base font-mono font-bold text-bone-base mb-3">Links</h2>
