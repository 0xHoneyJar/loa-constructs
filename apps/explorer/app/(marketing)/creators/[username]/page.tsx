  ...
  if (!creator) {
    return (
      <div className="py-24 text-center">
        <h1 className="text-lg font-mono text-bone-base mb-2">Creator Not Found</h1>
        <p className="text-sm font-mono text-bone-dim">No creator with username &quot;{username}&quot;.</p>
        <Link href="/constructs" className="text-xs font-mono text-bone-ghost hover:text-bone-bright mt-4 block">
          ← Browse constructs
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      <div className="flex items-center gap-4">
        <div className="h-16 w-16 border border-bone-ghost flex items-center justify-center text-2xl font-mono text-bone-dim">
      ...
      <div className="grid grid-cols-2 gap-4 text-xs font-mono">
        <div className="border border-void-border p-3">
          <p className="text-bone-ghost mb-1">Reputation</p>
          <p className="text-white text-lg">{creator.reputation}</p>
        </div>
        <div className="border border-void-border p-3">
          <p className="text-bone-ghost mb-1">Trust Score</p>
          <p className="text-white text-lg">{creator.trustScore}%</p>
        </div>
      </div>

      <div>
        <h2 className="text-sm font-mono font-bold text-bone-base mb-3">
          Published Constructs ({creator.constructs.length})
        </h2>
        {creator.constructs.length === 0 ? (
          <p className="text-xs font-mono text-bone-ghost">No published constructs yet.</p>
        ) : (
          <div className="space-y-2">
            {creator.constructs.map((c) => (
              ...
              >
                ...
                <span className="text-[10px] font-mono text-bone-ghost">{c.downloads.toLocaleString()} ↓</span>
              </Link>
            ))}
          </div>
        )}
