...
/**
 * GET /v1/skills
 * List skills with search, filter, pagination
 * @see sdd.md §5.3 GET /v1/skills
 */
skillsRouter.get('/', optionalAuth(), zValidator('query', listSkillsSchema), async (c) => {
 ...
 * GET /v1/skills/:slug
 * Get skill details
 * @see sdd.md §5.3 GET /v1/skills/:slug
 */
skillsRouter.get('/:slug', optionalAuth(), async (c) => {
  ...
  // Get latest version
  const latestVersion = await getLatestVersion(skill.id);

  // Get owner info
  const owner = await getSkillOwner(skill);

  logger.info({ request_id: requestId, slug }, 'Skill retrieved');
  ...
  return c.json({
    ...
    documentation_url: skill.documentationUrl,
    ...
    latest_version: latestVersion
      ? {
          ...
          min_loa_version: latestVersion.minLoaVersion,
